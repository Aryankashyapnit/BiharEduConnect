...
const collegeNameMap = {
  ...
  "R.R.D.C.E. BEGUSARAI": "RRSDCE-BEGUSARAI",
  ...
  "COLLEGE OF AGRICULTURAL ENGINEERING, ARA, BHOJPUR": "CAE-ARA-BHOJPUR" // Adding agricultural college if needed
};
...
function mapBranch(college, courseName) {
  ...
  if (course === "COMPUTER SC. & ENGINEERING" || course === "COMPUTER SC. & ENGG." || course === "COMPUTER SCIENCE & ENGINEERING" || course === "COMPUTER SC AND ENGG" || course === "COMPUTER SC. & ENGINEERING") {
  ...
  if (course === "ELECTRONICS & COMMUNICATION ENGINEERING" || course === "ELECTRO & COMMUNICATION ENGINEERING" || course === "ELECTRO & COMMUNICATION ENGG" || course === "ELECTRO & COMMUNICATON ENGINEERING") {
  ...
  if (course.includes("AGRICULTURE")) {
    if (branches.includes("AG")) return "AG";
  }
...
function parseBceceFile() {
  console.log(`\n--- Starting parsing for BCECE-2025 ---`);
  
  // CAE-ARA-BHOJPUR isn't in collegesData by default, so we can mock/add it
  let colleges;
  try {
    colleges = getCollegesData();
  } catch (e) {
    console.error("Colleges load error, using fallback mock", e);
    colleges = [];
  }
  
  // Ensure CAE-ARA-BHOJPUR exists in colleges list
  if (!colleges.some(c => c.code === "CAE-ARA-BHOJPUR")) {
    colleges.push({
      id: "cae-ara-bhojpur",
  ...
  const rawTextPath = path.join(__dirname, "raw_bcece_2025.txt");
  ...
  lines.forEach((line, index) => {
    ...
    if (!trimmed) return;
    if (trimmed.startsWith("Page No:-") || trimmed.startsWith("INSTITUTE") || trimmed.startsWith("FIRST ROUND") || trimmed.startsWith("FIRST  ROUND") || trimmed.startsWith("BRANCH") || trimmed.startsWith("SEAT TYPE")) return;
    ...
    if (numericParts.length >= 2) {
      ...
      // Pushing duplicate for round 2 so both filter queries work
      results.push({
        id: `c-bcece-2025-${idCounter++}`,
        collegeCode,
        branchCode,
  ...
  console.log(`Generated BCECE cutoff records: ${results.length}`);

  const outputPath = path.join(__dirname, "../src/data/bceceCutoffs2025.json");
  fs.writeFileSync(outputPath, JSON.stringify(results, null, 2));
  console.log(`Wrote official BCECE JSON results to ${outputPath}`);
}
